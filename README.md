...
Training-day headers show the complete muscle collection configured on the active cycle instead of a generic cycle description. Each circuit header shows the subset of those muscles worked by its current exercises, and that subset updates automatically after exercise swaps, favorites, or regeneration.

Workout settings define a continuous rotation of 1–16 entries instead of binding targets to weekdays. **+ Edit splits** opens the split manager and identifies the currently applied split. The supplied ULPPL, PPL, bro split, full body, high-frequency full body, 6-day upper/lower, and 4-day upper/lower with arms/delts splits remain immutable templates. Editing one creates a separately named custom split. Each split is an ordered Day 1…Day n list; every Day position can be assigned a reusable cycle or rest day, added, removed, or rearranged. **Begin week on** maps Day 1 to a calendar weekday only when the split is applied, so weekday names do not become part of the split itself.

The split manager's **Edit cycles** panel contains the reusable cycle library. A cycle has a name and a selectable collection of target muscles, and the same cycle may appear in multiple positions within a split. Editing a supplied cycle template creates a custom cycle, while **+ Add cycle** creates a new reusable cycle directly in this panel. An exercise qualifies for a cycle when it works at least one of that cycle's selected muscles.

Training and scheduled-rest entries can be removed, named, and reordered. Every workout and rest-day header has a rotation selector beside its day controls. Selecting another entry skips directly to it, regenerates that day and later days, and continues with the following rotation entry—including a scheduled rest day. Selecting a workout cycle on a rest day trains that cycle. A scheduled rest day consumes its place in the rotation; the separate **Rest today** control defers the pending entry without consuming it. The cycle summary includes an expandable body-part coverage list showing how many training cycles target each body part; rest, warm-up, and cool-down items are excluded from those counts. The global warm-up and cool-down checklists can be added to, renamed, reordered, or emptied from Settings. Warm-ups appear before the circuits; cool-downs appear after the circuits and are excluded from workout time. **5 core** is the single default cool-down item. The seven weekday slots consume the rotation in order, and the rotation continues across week boundaries.
...
The preset split picker can replace this default with ULPPL, PPL, bro split, full-body, high-frequency full-body, 6-day upper/lower, or 4-day upper/lower plus arms/delts rotations. Preset rest entries are part of the rotation and are distinct from calendar-day rest overrides.
