...
Each circuit starts with its **Total Body activator**, followed by sticky checkboxes for rounds 1–3, then two to four in-round exercises: **Focused** has 2, **Standard** has 3, and **Challenge** has 4. Use the `−` and `+` controls in a circuit heading to remove or add a compatible exercise. The day sidebar also stays visible while the workout scrolls.
